...
public class PuttingCourseGenerator {
	 ...
	 * smooth surface)<br>This is different from the smoothing_factor as a high roughness will also result in a large amount of tiny ugly deviations which can be mitigated with the
	 * smoothing_factor.<p>
	 * @param minimum_flag_distance
	 * @param hole_tolerance
	 * @param maximum_velocity
	 * @return
	 */
	public PuttingCourse fractalGeneratedCourse(int desired_size, int smoothing_factor, double roughness_height, double roughness_friction, double minimum_flag_distance, double hole_tolerance, double maximum_velocity) {
		...
		applyRangeToMatrix(fractal_h, -5, 20); // TODO finallize a better range or make it parametric
		applyRangeToMatrix(fractal_f, -2, 10); // TODO finallize a better range or make it parametric
		...
		Vector2d[] pos = determineFlagAndStartPositions(heightmap, frictionmap, minimum_flag_distance);
		return new PuttingCourse(height, friction, heightmap.length, heightmap[0].length, pos[0], pos[1], hole_tolerance, maximum_velocity);
	}
	
	private Vector2d[] determineFlagAndStartPositions(double[][] heightmap, double[][] frictionmap, double minimum_flag_distance) {
		return new Vector2d[] {
		/* flag  */	new Vector2d(random.nextInt(heightmap.length/2) + heightmap.length/3 + 0.5, random.nextInt(heightmap.length/2) + heightmap.length/3 + 0.5),
		/* start */	new Vector2d(heightmap.length/2,0)
		};
	}
